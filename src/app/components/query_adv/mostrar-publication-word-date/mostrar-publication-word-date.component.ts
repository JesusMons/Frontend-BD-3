...
@Component({
  selector: 'app-filtered-publications', // Component selector used in the HTML template
  ...
  templateUrl: './mostrar-publication-word-date.component.html', // HTML template path
  styleUrls: ['./mostrar-publication-word-date.component.css'] // CSS styling path
})

// Component class definition
export class PublicationFilterByWordDateComponent implements OnInit {
  ...
  // Method to load filtered publications from the service
  loadFilteredPublications() {
    this.publicationsService.filterPublications() // Calls the filterPublications method from the service
      .subscribe({
        next: (data) => {
